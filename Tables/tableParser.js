...
(function ($) {
    $(document).ready(function () {
        ...
        nbErrorFound = 0,
            ...
            techniqueURL = ["keycell-techniques.html", // 1
            ...
            techniqueName = ["Defining a Key Cell", // 1
        ...
        optimizeAccessibilitySet = function () {
                ...
                if (tblparser.theadRowStack.length !== 2) {
                    ...
                    if (ScopeColgroupEligible) {
                        for (i; i < tblparser.theadRowStack[0].cell.length; i += 1) {
                            if (tblparser.theadRowStack[0].cell[i].colgroup.start === tblparser.theadRowStack[0].cell[i].colgroup.end || tblparser.theadRowStack[0].cell[i].colgroup.start !== tblparser.theadRowStack[0].cell[i].colpos || tblparser.theadRowStack[0].cell[i].colgroup.end !== (tblparser.theadRowStack[0].cell[i].colpos + tblparser.theadRowStack[0].cell[i].width - 1) || tblparser.theadRowStack[0].cell[i].colgroup.type !== 2) {
...
tblparser.colgroup.length === 1)*/
                ) { // One data colum group

                    // Without grouping
                    if (tblparser.theadRowStack.length <= 1 && (tblparser.lstrowgroup.length === 1 || (tblparser.lstrowgroup[0].type === 2 && tblparser.lstrowgroup[1].type === 3 && tblparser.lstrowgroup[1].level === 0))) { // Only one row group or one data group with a summary group at level 0 (tfoot)
...
*/
            ...
            for (i = 0; i < tblparser.theadRowStack.length; i += 1) {
                ...
                for (j = 0; j < currRow.cell.length; j += 1) {
                    ...
                    if ((currCell.type === 1 || currCell.type === 7) && (!(j > 0 && currCell.uid === currRow.cell[j - 1].uid) && !(i > 0 && currCell.uid === tblparser.theadRowStack[i - 1].cell[j].uid))) {
                ...
                for (j = 0; j < currRow.cell.length; j += 1) {
                    ...
                    if ((j === 0) || (j > 0 && currRow.cell[j].uid !== currRow.cell[(j - 1)].uid)) {
                        ...
                        if (currCell.type === 1) {
                            ...
                            $(currCell.elem).attr('headers', (coldataheader ? coldataheader : '') + (currRow.idsheaderset && coldataheader ? ' ' : '') + (currRow.idsheaderset ? currRow.idsheaderset : '') + ((currRow.idsheaderset && ongoingRowHeader) || (coldataheader && ongoingRowHeader && !currRow.idsheaderset) ? ' ' : '') + (ongoingRowHeader ? ongoingRowHeader : ''));
        ...
        $('#analysetable').click(function () {
            ...
            if (_pe.fn.parsertable) {
                ...
                $(document).on('depsTableParserLoaded', optimizeAccessibilitySet);
                _pe.wb_load({
                    'dep': ['parserTable']
                }, "depsTableParserLoaded");
            }
        });
